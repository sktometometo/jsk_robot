...
class BehaviorManagerNode(object):
    ...
    def callback_synchronizer(self, msg):
        rospy.loginfo(
            "Current node is updated to {}".format(msg.feedback.current_node_id)
        )
    ...
    def navigate_edge(self, edge):
        ...
        if self.current_node_id != edge.node_id_from:
            rospy.logwarn(
                "current_node_id {} does not match node_id_from of edge ({})".format(
                    self.current_node_id, edge.node_id_from
                )
            )
            return False
        ...
        try:
            ...
            behavior = behavior_class(self.spot_client, self.sound_client)
        except Exception as e:
            rospy.logerr(
                "Failed to load and initialize behavior class: {}".format(
                    sys.exc_info()
                )
            )
            traceback.print_exc()
